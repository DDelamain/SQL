...
--CAST(value AS TYPE); Явное преобразование в желаемый тип;
--DATENAME(PART,date); Возвращает название фрагмента даты;
--DATEPART(PART,date); Возвращает числовое значение фрагмента даты;
...
--IFF(CONDITION, value_1, value_2); Conditional Ternary Operator: 
--Если условие (CONDITION) вернуло TRUE, то IIF возвращает значение 'vaalue_1', в противном случае - 'value_2'
...
SELECT
		...
		[Дата] = [date],
		[Дата недели] = DATENAME(WEEKDAY, [date]),
		[Время] = [time],
